...
class RPN_FUSION(nn.Module):
    ...
    def forward(self, x, f_view, f_view_r, idxs_norm,idxs_norm_ori,idxs_norm_ori_r, bev=None):
        # import pdb; pdb.set_trace()
        ups = []
        for i in range(len(self.blocks)):
            x = self.blocks[i](x)
        ...
        f1 = self.maxpool(F.relu(self.bn1(self.conv1(f_view_r))))
        f2 = self.layer1(f1)
        f3 = self.layer2(f2)
        f4 = self.layer3(f3)
        f5 = self.layer4(f4)
        f_view_features = self.fpn([f3, f4, f5])
        fuse_features2 = F.relu(f_view_features[0]).permute(0,1,3,2)
        fuse_features_r = fuse_features2.permute(0,1,3,2)
        ...
        #     mask_200 = torch.mul(idxs[:,1] > 0, idxs[:,1] < 200).unsqueeze(-1).to(torch.float)
        #     mask_176 = torch.mul(idxs[:,0] > 0, idxs[:,0] < 176).unsqueeze(-1).to(torch.float)
        #     idxs = idxs * mask_200 * mask_176
        #     # import pdb; pdb.set_trace()
        ...
        crop_feature_all_r = []
        for i in range(idxs_norm_ori_r.size()[1]):
            crop_feature_0 = feature_crop(fuse_features_r, idxs_norm_ori_r[:,i,:,:])
            crop_feature_all_r.append(crop_feature_0)
        crop_feature_all_r = torch.stack(crop_feature_all_r, dim=2)
        N, C, D, H, W = crop_feature_all_r.shape
        crop_feature_all_r = crop_feature_all_r.view(N,C*D,H,W)



        # l_ref = crop_feature_all.sum(1).view(N,H*W)
        # r_ref = crop_feature_all_r.sum(1).view(N,H*W)
        # lr_sum = (l_ref+r_ref).sort(descending=True)[1]
        # lr_sum = lr_sum[:,:1000]
        # for i in range(N):
        #     l_mask = torch.zeros([1,H*W]).cuda()
        #     l_mask[:, lr_sum[i]] = 1
        #     l_ref[i, :] = l_ref[i, :]*l_mask
        #     r_ref[i, :] = r_ref[i, :]*l_mask
        # l_ref = l_ref.view(N,1,H,W)
        # r_ref = r_ref.view(N,1,H,W)

        # import pdb; pdb.set_trace()
        crop_feature_l = self.rgb_refine(crop_feature_all)
        crop_feature_r = self.rgb_refine(crop_feature_all_r)
        # import pdb; pdb.set_trace()
        crop_gate_ref = torch.cat((crop_feature_l,crop_feature_r), 1)
        ...
        # layer_list.append(crop_feature_all_r.cpu().detach().numpy())
        ...
        # layer_list.append(l_ref.cpu().detach().numpy())
        # layer_list.append(r_ref.cpu().detach().numpy())
        # layer_name = ['left', 'right', 'fuse_rgb','bev','gated','l_ref','r_ref']
        # fol_name='vis_stereo/feature_vis'
